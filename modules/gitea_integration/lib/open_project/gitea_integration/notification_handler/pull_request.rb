...
module OpenProject::GiteaIntegration
  module NotificationHandler
    ##
    # Handles gitea pull request notifications.
    class PullRequest
      ...
      def process(params)
        ...
        work_packages = find_mentioned_work_packages(payload.pull_request.body, gitea_system_user)
      ...
      def already_referenced_work_packages(payload)
        pull_request = GiteaPullRequest
          .where(gitea_id: payload.pull_request.id)
          .or(GiteaPullRequest.where(gitea_html_url: payload.pull_request.html_url))
          .take
      ...
      def upsert_pull_request(payload, work_packages)
        OpenProject::GiteaIntegration::Services::UpsertPullRequest.new.call(payload.pull_request.to_h,
                                                                             work_packages:)
      end

...
module OpenProject::GiteaIntegration
  ##
  # Handles gitea-related notifications.
  # Each method is named after their webhook "event" as documented here:
  # https://docs.gitea.com/en/developers/webhooks-and-events/webhook-events-and-payloads
  module NotificationHandler
    class << self
      def check_run(payload)
        with_logging do
          OpenProject::GiteaIntegration::NotificationHandler::CheckRun.new.process(payload)
        end
      end

      def issue_comment(payload)
        with_logging do
          OpenProject::GiteaIntegration::NotificationHandler::IssueComment.new.process(payload)
        end
      end

      def pull_request(payload)
        with_logging do
          OpenProject::GiteaIntegration::NotificationHandler::PullRequest.new.process(payload)
        end
      end

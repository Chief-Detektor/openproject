...
module OpenProject::GiteaIntegration::Services
  ...
  # If the `GiteaPullRequest` already exists, it is updated.
  #
  # Returns the upserted `GiteaPullRequest`.
  #
  ...
  class UpsertPullRequest
    ...
    def find_or_initialize(payload)
      GiteaPullRequest.find_by_gitea_identifiers(id: payload.fetch('id'),
    ...
    def extract_params(payload)
      {
        ...
        gitea_user: gitea_user_id(payload.fetch('user')),
        number: payload.fetch('number'),
        gitea_html_url: payload.fetch('html_url'),
        gitea_updated_at: payload.fetch('updated_at'),
        ...
        merged_by: gitea_user_id(payload['merged_by']),

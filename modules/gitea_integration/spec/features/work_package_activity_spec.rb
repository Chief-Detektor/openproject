...
RSpec.describe 'Work Package Activity Tab',
               ...
               :js,
               :with_cuprite do
  shared_let(:gitea_system_user) { create(:admin, firstname: 'Gitea', lastname: 'System User') }
  shared_let(:admin) { create(:admin) }

  shared_let(:project) { create(:project, enabled_module_names: Setting.default_projects_modules + %w[activity]) }
  shared_let(:work_package) { create(:work_package, project:) }

  shared_let(:pull_request_author) { create(:gitea_user, gitea_login: 'i_am_the_author') }
  shared_let(:pull_request_merging_user) { create(:gitea_user, gitea_login: 'i_merged') }
  shared_let(:pull_request) do
    create(:gitea_pull_request,
           gitea_user: pull_request_author)
  end

  def trigger_pull_request_action
    OpenProject::GiteaIntegration::NotificationHandler::PullRequest.new
                                                                    .process(payload)
  ...
  let(:payload) do
    {
      'action' => action,
      'open_project_user_id' => gitea_system_user.id,
  ...
  let(:pull_request_hash) do
    {
      ...
      'html_url' => pull_request.gitea_html_url,
      ...
      'user' => {
        'id' => pull_request_author.gitea_id,
        'login' => pull_request_author.gitea_login,
        'html_url' => pull_request_author.gitea_html_url,
        'avatar_url' => pull_request_author.gitea_avatar_url
      },
      'base' => {
        'repo' => {
          'full_name' => 'author_user/repo',
          'html_url' => 'gitea.com/author_user/repo'
        }
      }
    }
  end
  ...
  context 'when the pull request is merged' do
    ...
    context "and I visit the work package's activity tab" do
      ...
      let(:merged_by_section_hash) do
        {
          'id' => pull_request_merging_user.gitea_id,
          'login' => pull_request_merging_user.gitea_login,
          'html_url' => pull_request_merging_user.gitea_html_url,
          'avatar_url' => pull_request_merging_user.gitea_avatar_url
        }
      end
      ...
      it 'renders a comment stating the Pull Request was merged by the merge actor' do
        expected_merge_comment = <<~gitea_MERGE_COMMENT.squish
          Merged#{I18n.t('js.gitea_integration.pull_requests.merged_message',
                         pr_number: pull_request.number,
                         pr_link: pull_request.title,
                         repository_link: pull_request.repository,
                         pr_state: 'merged',
                         gitea_user_link: pull_request_merging_user.gitea_login)}
        gitea_MERGE_COMMENT
  ...
  context 'when the Work Package is referenced in a Pull Request' do
    ...
    context "and I visit the work package's activity tab" do
      ...
      it 'renders a comment stating the Work Package was referenced in the Pull Request' do
        expected_referenced_comment = <<~gitea_REFERENCED_COMMENT.squish
          Referenced#{I18n.t('js.gitea_integration.pull_requests.referenced_message',
                             pr_number: pull_request.number,
                             pr_link: pull_request.title,
                             repository_link: pull_request.repository,
                             pr_state: 'referenced',
                             gitea_user_link: pull_request_author.gitea_login)}
        gitea_REFERENCED_COMMENT
  ...
  context 'when any non-edge-case action is performed on a pull request' do
    ...
    context "and I visit the work package's activity tab" do
      ...
      it 'renders a comment stating that said action was performed on the Pull Request' do
        expected_action_comment = <<~gitea_READY_FOR_REVIEW_COMMENT.squish
          Marked Ready For Review#{I18n.t('js.gitea_integration.pull_requests.message',
                                          pr_number: pull_request.number,
                                          pr_link: pull_request.title,
                                          repository_link: pull_request.repository,
                                          pr_state: 'marked ready for review',
                                          gitea_user_link: pull_request_author.gitea_login)}
        gitea_READY_FOR_REVIEW_COMMENT

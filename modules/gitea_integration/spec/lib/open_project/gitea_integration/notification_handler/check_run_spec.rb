...
RSpec.describe OpenProject::GiteaIntegration::NotificationHandler::CheckRun do
  subject(:process) { described_class.new.process(payload) }

  let(:upsert_check_run_service) do
    instance_double(OpenProject::GiteaIntegration::Services::UpsertCheckRun)
  end
  let(:payload) do
    {
      'check_run' => {
  ...
  before do
    allow(OpenProject::GiteaIntegration::Services::UpsertCheckRun).to receive(:new)
  ...
  context 'when the check_run is not associated to a known GiteaPullRequest' do
  ...
  context 'when the check_run is associated to a known GiteaPullRequest' do
    let(:pull_requests_payload) do
      [
        {
          'id' => 123

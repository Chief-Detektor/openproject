...
RSpec.describe OpenProject::GiteaIntegration::NotificationHandler::PullRequest do
  ...
  let(:upsert_service) { OpenProject::GiteaIntegration::Services::UpsertPullRequest.new }
      ...
      'pull_request' => {
        ...
        'base' => {
          'repo' => {
            ...
            'html_url' => 'gitea.com/test_user/repo'
          }
        }
      },
      'sender' => {
  ...
  let(:pr_merged) { false }
  let(:pr_draft) { false }
  let(:gitea_pull_request) { GiteaPullRequest.find_by_gitea_identifiers id: 123 }

  before do
    allow(handler_instance).to receive(:comment_on_referenced_work_packages).and_return(nil)
    allow(OpenProject::GiteaIntegration::Services::UpsertPullRequest).to receive(:new).and_return(upsert_service)
  ...
  context 'with a closed action when the PR was merged' do
    ...
    context 'when the work package is already known to the GiteaPullRequest' do
      let!(:gitea_pull_request) { create(:gitea_pull_request, gitea_id: 123, work_packages: [work_package]) }
  ...
  context 'with an edited action' do
    ...
    context 'when a GiteaPullRequest exists that is not linked to the mentioned work package yet' do
      let!(:gitea_pull_request) { create(:gitea_pull_request, gitea_id: 123) }
    ...
    context 'when the a work package is already known to the GiteaPullRequest but another work package is new' do
      let!(:gitea_pull_request) { create(:gitea_pull_request, gitea_id: 123, work_packages: [work_package]) }

...
RSpec.describe OpenProject::GiteaIntegration::HookHandler do
  subject(:process_webhook) do
    described_class.new
    .tap { journal_counts_before }
    .process('gitea', OpenStruct.new(env: environment), ActionController::Parameters.new(payload), user)
  ...
  let(:environment) do
    {
      ...
      'HTTP_X_GITEA_DELIVERY' => 'test delivery'
    }
  end
  ...
  shared_examples_for 'it does not create a pull request' do
    it 'creates no GiteaPullRequest or GiteaUser' do
      expect { process_webhook }.to(
        change(GiteaPullRequest, :count).by(0).and(
          change(GiteaUser, :count).by(0)
        )
      )
    end
  end

  shared_examples_for 'it creates a pull request and gitea user' do
    it 'creates a GiteaPullRequest and a GiteaUser' do
      expect { process_webhook }.to(change(GiteaPullRequest, :count).by(1).and(change(GiteaUser, :count).by(1)))

      pull_request = GiteaPullRequest.last
      gitea_user = GiteaUser.last
      ...
      expect(gitea_user).to have_attributes(
        gitea_login: 'test_user',
        gitea_html_url: 'https://gitea.com/test_user',
        gitea_avatar_url: 'https://avatars.giteausercontent.com/u/206108?v=4'
      )
    end
  end

  shared_examples_for 'it creates a partial pull request' do
    it 'creates a partial GiteaPullRequest' do
      expect { process_webhook }.to(
        change(GiteaPullRequest, :count).by(1).and(
          change(GiteaUser, :count).by(1)
        )
      )
  ...
  context 'when receiving a webhook for a pull_request event' do
    ...
    context 'when opened and mentioning a work package using its code in the PR body' do
      ...
      it_behaves_like 'it creates a pull request and gitea user'
    end
    ...
    context 'when the PR was merged' do
      ...
      it 'sets the merged_by relation to a GiteaUser' do
        process_webhook

        expect(GiteaPullRequest.last.merged_by.gitea_login).to eq 'test_user'
      end
    end
  end
  ...
  context 'when receiving a webhook for a check_run event' do
    let(:event) { 'check_run' }
    # gitea_id comes from the fixture files
    let(:gitea_pull_request) { create(:gitea_pull_request, :open, gitea_id: 606508565) }

    before { gitea_pull_request }
    ...
    context 'when a run was queued but is not associated to a PR' do
      ...
      it 'does not create a check run' do
        expect { process_webhook }.not_to(change(GiteaCheckRun, :count))
      end
    end
    ...
    context 'when a run was queued' do
      ...
      it 'creates a check run' do
        expect { process_webhook }.to(change(GiteaCheckRun, :count).by(1))
        check_run = GiteaCheckRun.last
        expect(check_run).to have_attributes(
          gitea_pull_request:,
          gitea_app_owner_avatar_url: 'https://avatars.giteausercontent.com/u/9919?v=4',
          status: 'queued',
          details_url: 'https://gitea.com/test_user/webhooks_playground/runs/2241417510',
          conclusion: nil
        )
      end
    end
    ...
    context 'when a run was completed successfully' do
      ...
      it 'creates a check run' do
        ...
        expect(check_run).to have_attributes(
          ...
          gitea_app_owner_avatar_url: 'https://avatars.giteausercontent.com/u/9919?v=4',
          status: 'completed',
          details_url: 'https://gitea.com/test_user/webhooks_playground/runs/2241431836',
          conclusion: 'success'
        )
      end
    end
    ...
    context 'when a run was completed with a failure' do
      ...
      it 'creates a check run' do
        ...
        expect(check_run).to have_attributes(
          ...
          name: 'test',
          status: 'completed',
          details_url: 'https://gitea.com/test_user/webhooks_playground/runs/2241416592',
          conclusion: 'failure'
        )
      end
    end

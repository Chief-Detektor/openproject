...
describe('PullRequestComponent', () => {
  ...
  const pullRequestStub = {
    ...
    deletionsCount: 3,
    draft: false,
    giteaUpdatedAt: 'test giteaUpdatedAt',
    htmlUrl: 'test htmlUrl',
    id: 3,
    labels: ['test'],
    merged: false,
  ...
  it('should render pull request data', () => {
    ...
    expect(avatarElement.src).toContain(pullRequestStub.giteaUser.avatarUrl);
    expect(userElement.textContent).toContain(pullRequestStub.giteaUser.login);

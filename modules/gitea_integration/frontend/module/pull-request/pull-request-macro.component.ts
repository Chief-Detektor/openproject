...
import { IGiteaPullRequest, IGiteaUserResource } from '../state/gitea-pull-request.model';
...
export const giteaPullRequestMacroSelector = 'macro.gitea_pull_request';

@Component({
  selector: giteaPullRequestMacroSelector,
...
export class PullRequestMacroComponent implements OnInit {
  ...
  pullRequest$:Observable<IGiteaPullRequest>;
  ...
  constructor(
    ...
    readonly pullRequests:GiteaPullRequestResourceService,
  ...
  private buildText(pr:IGiteaPullRequest):string {
    const actor = this.deriveActor(pr) as IGiteaUserResource;
    ...
    return this.I18n.t(
      `js.gitea_integration.pull_requests.${message}`,
      {
        pr_number: pr.number,
        pr_link: prLink,
        repository_link: repositoryLink,
        pr_state: this.I18n.t(
          `js.gitea_integration.pull_requests.states.${this.pullRequestState}`,
          { defaultValue: this.pullRequestState || '(unknown state)' },
        ),
        gitea_user_link: actorLink,
      },
    );
  }


  private deriveActor(pr:IGiteaPullRequest) {
    if (this.pullRequestState === 'merged') {
      return pr._embedded.mergedBy;
    } else {
      return pr._embedded.giteaUser;
    }
  }
